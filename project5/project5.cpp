...
const char *WINDOWTITLE = "Project 5 -- Peter LaMontagne";
...
int		FragOn;					// != 0 means to turn on the custom fragment shader
int		VertexOn;				// != 0 means to turn on the custom vertex shader
...
GLSLProgram	*Pattern;			// Our custom GLSL shader
...
Display( )
{
	...
	Pattern->SetUniformVariable( "uKd", .6f );
	Pattern->SetUniformVariable( "uKs", .3f );
	Pattern->SetUniformVariable( "uColor", 1.f, .3f, .0f );
	Pattern->SetUniformVariable( "uSpecularColor", 1.f, 1.f, 1.f );
	Pattern->SetUniformVariable( "uShininess", 3.f );
